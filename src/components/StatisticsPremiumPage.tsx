...
interface StatisticsPremiumPageProps {
  onBack?: () => void;
  isDark?: boolean;
...
export function StatisticsPremiumPage({
  onBack,
  isDark = true,
  setIsDark,
  onNavigateToChat,
  subscriptionStatus,
}: StatisticsPremiumPageProps) {
  const [isProfileSettingsModalOpen, setIsProfileSettingsModalOpen] = useState(false);
  const { user } = useAuth();

  const plan = user?.plan ?? 'free';
  const planKey = plan?.toString().toLowerCase();
  const derivedStatus = user ? resolveStatus(plan, user?.is_premium) : subscriptionStatus ?? 'free';
